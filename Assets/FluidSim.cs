...
namespace EulerianFluidSimulator
{
    public class FluidSim
    {
        ...
        //sana
        public int To2D(int i, int j, int k ) => i + (numX * j) + (numX *numY * k);
        ...
        //sana
        public float SimDepth => numZ * h;

        //Is a coordinate within the simulation area?
        public bool IsWithinArea(float x, float y , float z) => (x > 0 && x < SimWidth && y > 0 && y < SimHeight && z > 0 && z < SimDepth);


        //sana
        public FluidSim(float density, int numX, int numY, int numZ , float h)
        {
        ...
        public void Simulate(float dt, float gravity, int numIters, float overRelaxation)
        {
          ...
          //  AdvectSmoke(dt); //sana commented this
                ...
                for (int k = 1; k < this.numZ - 1; k++)
                {
                    ...
                    for (int j = 1; j < this.numY - 1; j++)
                    {
                        for (int i = 1; i < this.numX - 1; i++)
                        {
                            ...
                            int indexThis = To2D(i, j, k);
                            int indexLeft = To2D(i - 1, j, k);
                            int indexRight = To2D(i + 1, j, k);
                            int indexBottom = To2D(i, j - 1, k);
                            int indexTop = To2D(i, j + 1, k);
                            int indexfront = To2D(i, j, k + 1);
                            int indexback = To2D(i, j, k - 1);
                           ...
                           // int  indexfront = To2D(i, j, k - 1);
                            ...
                            float divergence = u[indexRight] - u[indexThis] + v[indexTop] - v[indexThis] + w[indexfront] - w[indexThis];
        ...
        private void Extrapolate()
        {
            ...
            for (int i = 0; i < numX; i++)
            {
                ...
                u[To2D(i, 0,0)] = u[To2D(i, 1,1)];

                //Top border row gets the same velocity in u direction as the row below
                u[To2D(i, numY - 1 , numZ-1)] = u[To2D(i, numY - 2 ,numZ-2)];
            }
            ...
            for (int j = 0; j < numY; j++)
            {
                ...
                v[To2D(0, j,0)] = v[To2D(1, j,1)];

                //Right border column gets the same velocity in v direction as the column to the left 
                v[To2D(numX - 1, j, numZ-1)] = v[To2D(numX - 2, j, numZ-2)];
            }
            //sana
            for (int k = 0; k < numZ; k++)
            {
                
                w[To2D(0, 0,k)] = w[To2D(1, 1,k)];

               
                w[To2D(numX - 1, numY-1,k)] = w[To2D(numX - 2, numY-2,k)];
            }
        }
            ...
            for (int k = 1; k < this.numZ; k++)
            {
                for (int j = 1; j < this.numY; j++)
                {
                    for (int i = 1; i < this.numX; i++)
                    {
                        ...
                        if (this.s[To2D(i, j,k)] != 0f && this.s[To2D(i - 1, j,k)] != 0f && j < this.numY - 1
                            && k < this.numZ - 1)
                        {
                            ...
                            u = SampleField(x, y, z ,SampleArray.uField);
                     

                            this.uNew[To2D(i, j,k)] = u ;
                           
                        }
                        //Update v component
                        //v is on the border between 2 cells, so neither of them can be an obstacle
                        if (this.s[To2D(i, j,k)] != 0f && this.s[To2D(i, j - 1,k)] != 0f && i < numX - 1
                            && k < this.numZ - 1)
                        {
                            ...
                            v = SampleField(x, y, z,SampleArray.vField);
                           

                            this.vNew[To2D(i, j,k)] = v  ;
                        }

                        //sana
                        if (this.s[To2D(i, j,k)] != 0f && this.s[To2D(i, j ,k-1)] != 0f 
                             && i < numX - 1 && j < numY - 1)
                        {
                            ...
                            w = SampleField(x, y, z,SampleArray.wField);
                           

                            this.wNew[To2D(i, j,k)] = w ;
                        }
                    }
                }
            }
            ...
            for (int k = 1; k < this.numZ - 1; k++)
            {
                for (int j = 1; j < this.numY - 1; j++)
                {
                    for (int i = 1; i < this.numX - 1; i++)
                    {
                        ...
                        if (this.s[To2D(i, j,k)] != 0f)
                        {
                            //The velocity in the middle of the cell is the average of the velocities on the cell border
                            float u = (this.u[To2D(i, j,k)] + this.u[To2D(i + 1, j,k)]) * 0.5f;
                            float v = (this.v[To2D(i, j,k)] + this.v[To2D(i, j + 1,k)]) * 0.5f;
                            float w = (this.w[To2D(i, j,k)] + this.w[To2D(i, j ,k+1)]) * 0.5f;
                            ...
                            this.mNew[To2D(i, j,k)] = SampleField(x, y,z, SampleArray.smokeField);
                        }
                    }
                }
            }
        ...
        {
            float uTot = u[To2D(i, j - 1 ,k)] + u[To2D(i, j,k)] + u[To2D(i + 1, j - 1,k)] + u[To2D(i + 1, j,k)]+
                         u[To2D(i, j - 1, k-1)] + u[To2D(i, j, k-1)] + u[To2D(i + 1, j - 1, k-1)] + u[To2D(i + 1, j, k-1)];

            float uAverage = uTot * 0.125f;
        ...
        {
            float vTot = v[To2D(i - 1, j,k)] + v[To2D(i, j,k)] + v[To2D(i - 1, j + 1,k)] + v[To2D(i, j + 1,k)] +
                        v[To2D(i - 1, j, k-1)] + v[To2D(i, j, k-1)] + v[To2D(i - 1, j + 1, k-1)] + v[To2D(i, j + 1, k-1)];

            float vAverage = vTot * 0.125f;
        ...
        {
            float wTot = w[To2D(i, j-1,k)] + w[To2D(i, j,k)] + w[To2D(i - 1, j -1 ,k)] + w[To2D(i-1, j ,k)] +
                          w[To2D(i, j - 1, k+1)] + w[To2D(i, j, k+1)] + w[To2D(i - 1, j - 1, k+1)] + w[To2D(i - 1, j, k+1)];

            float wAverage = wTot * 0.125f;
        ...
        public float SampleField(float xP, float yP , float zP, SampleArray field)
        {
            GridConstants gridData = new(this.h, this.numX, this.numY , this.numZ);
            ...
            switch (field)
            {
                ...
                case SampleArray.wField: fieldToSample = GridInterpolation.Grid.w; arrayToSample = this.w; break;
            }
            ...
            GridInterpolation.ClampInterpolationPoint(xP, yP, zP ,gridData, fieldToSample, out float xP_clamped, out float yP_clamped, out float zP_clamped);


            //Figure out which values to interpolate between

            //Get the array index of A 
            GridInterpolation.GetAIndices(xP_clamped, yP_clamped, zP_clamped, gridData, fieldToSample, out int xA_index, out int yA_index , out int zA_index);

            //The values we want to interpolate between
            
            float A = arrayToSample[To2D(xA_index + 0, yA_index + 0 , zA_index +0)];
            float B = arrayToSample[To2D(xA_index + 1, yA_index + 0 , zA_index +0)];
            float C = arrayToSample[To2D(xA_index + 0, yA_index + 1, zA_index +0)];
            float D = arrayToSample[To2D(xA_index + 1, yA_index + 1, zA_index +0)];

            //Debug.Log("A");
           // Debug.Log(To2D(xA_index + 0, yA_index + 0, zA_index + 0));

            //Debug.Log(To2D(xA_index + 0, yA_index + 0, zA_index + 0));
            //sana

            float E = arrayToSample[To2D(xA_index + 0, yA_index + 0, zA_index + 1)];
            float F = arrayToSample[To2D(xA_index + 1, yA_index + 0, zA_index + 1)];
            float G = arrayToSample[To2D(xA_index + 0, yA_index + 1, zA_index + 1)];
            float H = arrayToSample[To2D(xA_index + 1, yA_index + 1, zA_index + 1)];
            ...
            GridInterpolation.GetACoordinates(fieldToSample, xA_index, yA_index, zA_index, gridData, out float xA, out float yA, out float zA);

            //The weights for the interpolation between the values
            GridInterpolation.GetWeights(xP_clamped, yP_clamped, zP_clamped, xA, yA, zA ,gridData, out float wA, out float wB, out float wC, out float wD
                //sana
                , out float wE, out float wF, out float wG, out float wH);
        ...
        //sana
        public float SampleFieldOld(float xp_pos, float yp_pos, float zp_pos, SampleArray field)
        {
            //Cellsize
            float h = this.h;
            ...
            //- v is stored in the middle of the horizontal cell lines
            //- smoke is stored in the center of each cell
            float minXOffset, maxXOffset, minYOffset, maxYOffset , minZOffset , maxZOffset;

            minXOffset = maxXOffset = minYOffset = maxYOffset = minZOffset = maxZOffset = halfH;

            switch (field)
            {
                case SampleArray.uField: minXOffset = 0f; maxXOffset = h; break;
                case SampleArray.vField: minYOffset = 0f; maxYOffset = h; break;
                case SampleArray.wField: minZOffset = 0f; maxZOffset = h; break;
            }
            ...
            zp_pos = Mathf.Max(Mathf.Min(zp_pos, this.numZ * h - maxZOffset), minZOffset);
            ...
            switch (field)
            {
                ...
                case SampleArray.wField: f = this.w; dz = halfH; break;
                case SampleArray.smokeField: f = this.m; dx = halfH; dy = halfH; dz= halfH ; break;
            }
            ...
            int z0_index = Mathf.Min(Mathf.FloorToInt((zp_pos - dz) * oneOverH), this.numZ - 2);
            ...
            // |   | |
            // A-----B
            float A = f[To2D(x0_index, y0_index, z0_index)];
            float B = f[To2D(x1_index, y0_index, z0_index)];
            float C = f[To2D(x0_index, y1_index, z0_index)];
            float D = f[To2D(x1_index, y1_index, z0_index)];
            //sana
            float E = f[To2D(x0_index, y0_index, z1_index)];
            float F = f[To2D(x1_index, y0_index, z1_index)];
            float G = f[To2D(x0_index, y1_index, z1_index)];
            float H = f[To2D(x1_index, y1_index, z1_index)];
