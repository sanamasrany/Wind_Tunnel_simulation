...
public static class GridInterpolation
{
    ...
    //sana
    public static void ClampInterpolationPoint(float xP, float yP, float zP, GridConstants gridData, Grid sampleField, out float xP_clamped, out float yP_clamped, out float zP_clamped)
    {
        float minXOffset, maxXOffset, minYOffset, maxYOffset, minZOffset, maxZOffset;

        minXOffset = maxXOffset = minYOffset = maxYOffset = minZOffset = maxZOffset= gridData.half_h;
;
        ...
        switch (sampleField)
        {
            ...
            case Grid.w: minZOffset = 0f; maxZOffset = gridData.h; break;
        }
        ...
        zP_clamped = Mathf.Min(zP, gridData.ZLength - maxZOffset);
        ...
        zP_clamped = Mathf.Max(zP_clamped, minZOffset);
    }
    ...
    //sana
    public static void GetAIndices(float xP, float yP , float zP, GridConstants gridData, Grid sampleField, out int xA_index, out int yA_index, out int zA_index)
    {
        GetGridOffsets(sampleField, gridData.half_h, out float dx, out float dy, out float dz);
        ...
        int cellIndexZ = Mathf.FloorToInt((zP - dz) * gridData.one_over_h);
       
       //Debug.Log("alawww" +cellIndexZ);
        ...
        zA_index = Mathf.Min(cellIndexZ, gridData.numZ - 2);
    }
    ...
    //sana
    public static void GetACoordinates(Grid sampleField, int xA_index, int yA_index, int zA_index, GridConstants gridData, out float xA, out float yA, out float zA)
    {
        GetGridOffsets(sampleField, gridData.half_h, out float dx, out float dy, out float dz);
    
        xA = xA_index * gridData.h + dx;
        yA = yA_index * gridData.h + dy;
        zA = zA_index * gridData.h + dz;
    }
    ...
    private static void GetGridOffsets(Grid sampleField, float half_h, out float dx, out float dy ,out float dz)
    {
        dx = 0f;
        dy = 0f;
        dz = 0f;
        ...
        switch (sampleField)
        {
            ...
            case Grid.w: dz = half_h; break;
            case Grid.center: dx = half_h; dy = half_h; dz = half_h; break;
        }
    }
}
